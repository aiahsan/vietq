...
const Discover = () => {
    ...
    useEffect(() => {
        setdisplay(true);
        console.log(location.state, "resss")
    ...
    ><div className="bgimgcovermap">
            ...
            <Map
                ...
                Mapbox
                zoom={[13]}
                center={[mapData.longitude!=""?mapData.longitude:108.2772, mapData.latitude!=""?mapData.latitude:14.0583]}
            >
                <Layer type="symbol" id="marker" layout={{ 'icon-image': 'marker-15' }}>
                    <Feature coordinates={[mapData.longitude, mapData.latitude]} />
                </Layer>
            </Map>
        </div>
        ...
        <div className="row pd6p ">
            {listings.map(x => <div className="col-md-4 mt-5">
                <PropertyCard  ptype={x&&x.prop_type?x.prop_type:""} item={x} img={x.images[0] ? x.images[0].image : "https://globalimpactnetwork.org/wp-content/themes/globalimpact/images/no-image-found-360x250.png"} title={x.title ? x.title : ""} location={`${x.address ? x.address : ""}`} fetures={x.attributes ? x.attributes : []} price={x.price ? x.price : ""} type={0} />
            </div>)}
